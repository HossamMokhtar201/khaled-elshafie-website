...
export default function ContentTabs({
  ...
  const filteredVideos = useMemo(
    () =>
      topic === "الكل" ? featuredVideos : featuredVideos.filter((v) => v.topics.includes(topic)),
    [featuredVideos, topic]
  );

  return (
    <div>
      <div role="tablist" aria-label="نوع المحتوى" className="mb-6 flex gap-2 border-b border-border">
        <button
          type="button"
          role="tab"
      ...
      <div className="mb-8 flex flex-wrap gap-2" role="group" aria-label="فلترة حسب الموضوع">
        {TOPICS.map((t) => (
          <button
            key={t}
            type="button"
