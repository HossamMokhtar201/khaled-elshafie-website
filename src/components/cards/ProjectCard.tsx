...
export default function ProjectCard({
  ...
  return (
    ...
    >
      <Badge tone={statusTone[status] ?? "neutral"}>{projectStatusLabels[status] ?? status}</Badge>
      <h3 className="mt-3 font-heading text-lg font-bold text-text-primary">{name}</h3>
